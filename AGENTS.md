...
| `src/downloader.py` | yt-dlp, gallery-dl | yt-dlp subprocess calls: `get_metadata()` (optional `format_selector` for accurate size estimates, 60s timeout, `--no-playlist`, logs stderr on failure, raises `DownloadAuthRequired` for age-restricted), `download_video()`, `download_audio()`, `download_images()`, `download_gallery_dl_images()`, `download_gallery_dl_video()`. Raises `DownloadAuthRequired` when content requires login (age-restricted YouTube, TikTok login-gated). Raises `DownloadError` on gallery-dl timeout (user_message + raw_error for logging). |
