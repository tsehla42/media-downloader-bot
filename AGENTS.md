...
| `src/handlers.py` | config, utils, downloader, logging_config | Telegram handlers with group detection, orchestrates download flow, logs requests |
...
5. `handlers.py` calls `downloader.download_video()` or `download_audio()` or `download_images()`
6. `handlers.py` sends file to Telegram, cleans up temp files
7. `@with_request_logging` decorator logs request lifecycle automatically:
