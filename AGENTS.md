...
**Tech stack:** Python 3.12+, python-telegram-bot, yt-dlp (subprocess), instaloader (Instagram images), pytest
...
|---|---|---|
| `src/config.py` | .env file | Loads BOT_TOKEN, ALLOWED_USER_IDS, ALLOWED_GROUP_IDS, DOWNLOAD_DIR, MAX_FILE_SIZE, LOG_OUTPUT, LOG_DIR |
...
| `src/downloader.py` | yt-dlp, instaloader | yt-dlp subprocess calls: get_metadata, download_video, download_audio, download_images. instaloader for Instagram image fallback: download_instagram_image |
...
5. If metadata fails for Instagram: fallback to `download_instagram_image()` using instaloader
...
All 62 tests use mocked subprocess calls - no real downloads needed.
