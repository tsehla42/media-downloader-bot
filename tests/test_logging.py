...
def test_log_guest_request_received_basic():
    ...
    caller = MagicMock()
    caller.id = 628055047
    caller.first_name = "Меменасе"
    caller.username = "memenac"

    chat = MagicMock()
    chat.id = -1003804964305
    chat.title = "mememedia test"
    chat.type = "supergroup"
    ...
    with patch("logging_config.requests_logger") as mock_logger:
        ...
        assert log_data["user"]["id"] == 628055047
        assert log_data["user"]["name"] == "Меменасе"
        assert log_data["user"]["username"] == "memenac"
        assert log_data["chat"]["id"] == -1003804964305
        assert log_data["chat"]["name"] == "mememedia test"
        assert log_data["chat"]["type"] == "supergroup"
...
def test_log_guest_request_received_with_reply():
    ...
    with patch("logging_config.requests_logger") as mock_logger:
        ...
        log_data = mock_logger.info.call_args[1]["extra"]["extra_data"]
        assert log_data["reply"]["user_id"] == 390026995
        assert log_data["reply"]["name"] == "tsehla from the dungeon"
        assert log_data["reply"]["username"] == "tsehla42"
